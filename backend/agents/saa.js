...
class SignalAnalysisAgent {
  ...
  async calculateDayOfCycle(userId) {
    try {
      ...
      const lastPeriodStart = new Date(result[0].event_date);
  ...
  async getAverageCycleLength(userId) {
    try {
      ...
      if (result.length < 2) return 28; // default
  ...
  async predictNextPeriod(userId) {
    try {
      ...
      const lastPeriod = await runQuery(
        'SELECT event_date FROM cycle_events WHERE user_id = ? AND event_type = ? ORDER BY event_date DESC LIMIT 1',
        [userId, 'period_start']
      );
      
      if (lastPeriod.length === 0) return null;
      
      const lastPeriodDate = new Date(lastPeriod[0].event_date);
